...
mod app {
    ...
    #[shared]
    struct Shared {
        pin: hal::gpio::Pin<hal::gpio::pin::bank0::Gpio8, hal::gpio::PushPullOutput>,
    }
    ...
    fn init(c: init::Context) -> (Shared, Local, init::Monotonics) {
        ...
        let mut pin = pins.gpio8.into_push_pull_output();
        pin.set_low().unwrap();
    ...
    fn blink_led(mut c: blink_led::Context) {
        if *c.local.tog {
            c.shared.pin.lock(|l| l.set_high().unwrap());
        } else {
            c.shared.pin.lock(|l| l.set_low().unwrap());
        }
